...
int main() {
    cv::Mat dstMat, bnyMat;
    cv::Mat srcMat = cv::imread("/Users/shuziqi/Desktop/rim.png",0);
    if(srcMat.empty()) return -1;
    
    std::vector<Mat>SrcMatpart(srcMat.channels());
    split(srcMat, SrcMatpart);

    threshold(SrcMatpart[0], dstMat, 100, 255, THRESH_BINARY_INV);

    
    vector<vector<Point>>contours;
    vector<Vec4i> hierarchy;
    findContours(dstMat, contours, CV_RETR_EXTERNAL, CV_CHAIN_APPROX_SIMPLE);
